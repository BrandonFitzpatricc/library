...
class Book {
  ...
  display() {
    ...
    removeBtn.addEventListener("click", (event) => {
      ...
      const removedBookObj = myLibrary.splice(
        myLibrary.findIndex(
          (book) => book.#id === removedBookDisplay.dataset.id,
        ),
        1,
      )[0];
      ...
      const removedBookWidth = bookWidths.find(
        (bookWidth) => bookWidth.book === removedBookObj,
      ).value;
      ...
      // appearance.
      const modifyingGridColumnSize =
        removedBookWidth === currentGridColumnSize &&
      ...
      if (modifyingGridColumnSize) {
        ...
        // second widest book.
        bookWidths.sort(
          (bookWidth1, bookWidth2) => bookWidth1.value - bookWidth2.value,
        );
        bookWidths.pop();
        currentGridColumnSize = bookWidths.at(-1).value;
        root.style.setProperty(
          "--grid-column-size",
          currentGridColumnSize + "px",
        );
      } else {
        bookWidths.splice(
          bookWidths.findIndex(
            (bookWidth) => bookWidth.value === removedBookWidth,
          ),
          1,
        );
      }
    ...
    const bookWidth = +getComputedStyle(book)
...
const validationFailedMessage = document.querySelector(
...
submitBookBtn.addEventListener("click", () => {
  ...
  if (addBookForm.checkValidity()) {
    ...
    // Max page number limit is 9999.
    const pagesValue = pages.value > 9999 ? 9999 : pages.value;

    const hasReadValue = hasRead.value;

    addBookToLibrary(titleValue, authorValue, pagesValue, hasReadValue);
    ...
    validationFailedMessage.textContent = missingFields
